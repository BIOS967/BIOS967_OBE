...
files.ids <- get.pdb(ids, split = TRUE)
```
...
```{r, message=FALSE, results='hide'}
# Alignment
pdbs <- pdbaln(files.ids, fit = TRUE, exefile="msa")
```
...
```{r}
pdbs$ali[1:nrow(pdbs$ali), c(94, 96, 106, 199)]
```
...
![](unnamed-chunk-11-1.png){width="537"}
...
![B-factor showing dynamics in our protein structure. Black bar (Alpha-helix), gray bar (beta-sheets)](unnamed-chunk-13-1.png){width="537"}
...
![](unnamed-chunk-15-1.png){width="348"}
...
![](unnamed-chunk-16-1.png){width="346"}
...
***DS: since this chunk was giving problems when knitting, I suggest running it separately & saving the results. Then, for knitting this document, you can just import the saved .rdata file and proceed***
...
***Here is how you would save the output (not run on knitting)***

```{r, eval=F}
save(pdbs, file="data/pdbs.rdata")
```

***And here is how you would load the saved output (run this)***
